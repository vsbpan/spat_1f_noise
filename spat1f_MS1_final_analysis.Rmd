...
subm_sl <- glmmTMB(
  sl_mean_obs_log_scale ~ 
    beta_numeric_scale + on_toxic + var_high + ava_qual * cat_pre_wt_log_scale + 
    cat_pre_wt_log_scale_sq + 
    (1|session_id),
  data = d2,
); summary(subm_sl)
...
subm_toxin_ingested <- glmmTMB(
  mean_toxic_conc_scale ~ 
    ...
    on_toxic + cat_pre_wt_log_scale_sq +  
    (1|session_id),
  data = d2 
...
sem_summary <- suppressWarnings(suppressMessages(
  summary_psem(sem_fit, 
               .progressBar = FALSE, 
               direction = "ava_qual -> RGR_scale")
  ))

# Standardize via observed-empirical method
sem_summary <- append_std_coef(sem_summary, 
                               resp = "on_toxic",
                               round(obs_emp_std(subm_on_toxic), digits = 4))
sem_summary <- append_std_coef(sem_summary, 
                               resp = "ava_qual", 
                               round(obs_emp_std(subm_ava), digits = 4))
```
