...
from detectron2.structures.keypoints_hack import heatmaps_to_keypoints2,heatmaps_modify_iterative, heatmaps_to_keypoints_iterative
...
def keypoint_rcnn_loss(pred_keypoint_logits, instances, normalizer):
    ...
    iterative_find = False
    if iterative_find:
      pred_keypoint_logits = heatmaps_modify_iterative(pred_keypoint_logits, None, pred_keypoint_logits.shape[0])
    else:
      pred_keypoint_logits = pred_keypoint_logits
...
def keypoint_rcnn_inference(pred_keypoint_logits: torch.Tensor, pred_instances: List[Instances]):
    ...
    if iterative_find:
      keypoint_results = heatmaps_to_keypoints_iterative(pred_keypoint_logits, bboxes_flat.detach())
    else:
